...
((bbn) => {
  return {
    ...
    methods: {
      ...
      makeMe(role){
        ...
        if ( this.canUnmakeMe && role && this.tasks.source.roles[role] ){
          $.each(this.source.roles, (i, v) => {
            if ( $.inArray(appui.app.user.id, v) > -1 ){
              exists = true;
            }
          });
      ...
      unmakeMe(force){
        ...
        const removeRole = () => {
          return bbn.fn.post(this.root + "actions/role/delete", {
            ...
            const idx = $.inArray(appui.app.user.id, this.source.roles[prop]);
            if ( idx > -1 ){
      ...
      addBudget(){
        if ( this.isAdmin ){
          bbn.vue.find(this, 'appui-task-widget-budget').showPriceForm = true;
        }
      },
      budgetButtons(){
        return !this.source.price && this.isAdmin ? [{
