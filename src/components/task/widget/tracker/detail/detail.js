...
(() => {
  return {
    ...
    methods: {
      ...
      renderStart(row){
        return bbn.dt(row.start).format('DD/MM/YYYY HH:mm:ss');
      },
      renderEnd(row){
        if ( !row.end ){
          return bbn._('In progress') + '...';
        }
        return bbn.dt(row.end).format('DD/MM/YYYY HH:mm:ss');
      },
      edit(row, col, idx){
      ...
      canEditOrDelete(row) {
        if ((appui.user.id === row.id_user)
          && (bbn.dt().diff(bbn.dt(row.end), 'hours') < 48)
        ) {
          return true;
        }
      ...
      <bbn-textarea bbn-model="source.row.message"
        ...
        data(){
          return {
            ...
            maxEnd: bbn.dt().format('YYYY-MM-DD HH:mm:ss')
          }
        },
        computed: {
          maxStart(){
            if (!this.source.row.end) {
              return bbn.dt().format('YYYY-MM-DD HH:mm:ss');
            }

            let end = bbn.dt(this.source.row.end).unix(),
                now = bbn.dt().unix();
            return end < now ? this.source.row.end : bbn.dt().format('YYYY-MM-DD HH:mm:ss');
          }
        },
        methods: {
          validation(){
            if ( bbn.dt(this.source.row.end).unix() < bbn.dt(this.source.row.start).unix() ){
        ...
        watch: {
          'source.row.start'(newVal){
            if ( newVal && this.source.row.end ){
              this.$set(this.source.row, 'length', bbn.dt(this.source.row.end).unix() - bbn.dt(this.source.row.start).unix());
            }
            else {
