            ...
            this.post(this.root + 'actions/role/' + (this.manage ? 'update' : 'insert'), obj, d => {
              if (d.success) {
                ...
                if (d.roles !== undefined) {
                  let comps = this.closest('appui-task-page').findAllByKey(this.source.id, 'appui-task-item');
                  if (comps.length) {
                      ...
                      if (d.success) {
                        if (d.roles && bbn.fn.numProperties(d.roles)) {
                          bbn.fn.iterate(d.roles, (roles, id) => {
                            ...
                            let comps = this.closest('appui-task-page').findAllByKey(id, 'appui-task-item');
                            if (comps.length) {
